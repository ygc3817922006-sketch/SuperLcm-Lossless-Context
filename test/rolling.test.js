...
test('rolling selection keeps the fresh tail and folds only the older head', () => {
  ...
  const selection = selectRollingRange(nodes, nodes.map(node => node.seq), { tailCount: 3, minRetainTokens: 30000, foldBatchTokens: 20000, pressureFoldTokens: 10000, softActiveTokens: 100000, hardActiveTokens: 120000, activeTokens: 80000 })
  assert.deepEqual(selection, { start: 1, end: 5, foldTokens: 50000, tailNodes: 3, tailTokens: 30000, tailCountRelaxed: false, activeTokens: 80000, reason: 'background-batch' })
})
...
test('routine batch permits detached preparation below pressure', () => {
  const nodes = Array.from({ length: 10 }, (_, index) => ({ seq: index + 1, tokens: 10000 }))
  const selection = selectRollingRange(nodes, nodes.map(node => node.seq), { tailCount: 3, minRetainTokens: 30000, foldBatchTokens: 64000, pressureFoldTokens: 20000, softActiveTokens: 160000, hardActiveTokens: 220000, activeTokens: 100000 })
  assert.equal(selection.reason, 'background-batch')
})

test('soft pressure admits a smaller useful fold', () => {
  const nodes = Array.from({ length: 10 }, (_, index) => ({ seq: index + 1, tokens: 18000 }))
  const selection = selectRollingRange(nodes, nodes.map(node => node.seq), { tailCount: 3, minRetainTokens: 32000, foldBatchTokens: 64000, pressureFoldTokens: 20000, softActiveTokens: 160000, hardActiveTokens: 220000, activeTokens: 180000 })
...
test('hard pressure forces any balanced non-empty foldable head', () => {
  ...
  const selection = selectRollingRange(nodes, nodes.map(node => node.seq), { tailCount: 2, minRetainTokens: 32000, pressureFoldTokens: 20000, foldBatchTokens: 64000, softActiveTokens: 160000, hardActiveTokens: 220000, activeTokens: 225000 })
...
test('hard pressure can relax the node-count tail without dropping the token floor', () => {
  ...
  const selection = selectRollingRange(nodes, nodes.map(node => node.seq), { tailCount: 24, minRetainTokens: 32000, pressureFoldTokens: 20000, foldBatchTokens: 64000, softActiveTokens: 160000, hardActiveTokens: 220000, activeTokens: 240000 })
  assert.equal(selection.reason, 'hard-cap')
...
test('rolling selection honors the token floor in addition to node count', () => {
  ...
  const selection = selectRollingRange(nodes, nodes.map(node => node.seq), { tailCount: 2, minRetainTokens: 40000, foldBatchTokens: 20000, activeTokens: 80000 })
...
test('rolling selection walks the boundary back to a balanced tool pair', () => {
  ...
  const selection = selectRollingRange(nodes, nodes.map(node => node.seq), { tailCount: 2, minRetainTokens: 32000, foldBatchTokens: 20000, activeTokens: 180000, isBalancedBefore: seq => seq === 2 })
...
test('rolling selection returns null for empty, mismatched, or insufficient spans', () => {
  ...
  assert.equal(selectRollingRange(nodes, nodes.map(node => node.seq), { tailCount: 2, minRetainTokens: 2000, foldBatchTokens: 20000, activeTokens: 4000 }), null)
})
