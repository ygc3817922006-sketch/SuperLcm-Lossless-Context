...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const DEFAULTS = Object.freeze({
      ...
      fallbackSummarizationRoute: Object.freeze({ provider: "", model: "" }),
    ...
    function resolvedDraft(draft) {
      return {
        ...
        fallbackSummarizationRoute: {
          ...DEFAULTS.fallbackSummarizationRoute,
          ...((draft && draft.fallbackSummarizationRoute) || {}),
        },
      };
    }
    ...
    function SummarizerPicker({ t, scope, snapshot, catalogState, reloadCatalog, field, optional, headingKey, hintKey }) {
      ...
      const current = normalizeRoute(snapshot.value && snapshot.value[field]);
      ...
      const commit = async (route) => {
        ...
        const otherField = field === "summarizationRoute" ? "fallbackSummarizationRoute" : "summarizationRoute";
        const other = normalizeRoute(snapshot.value && snapshot.value[otherField]);
        ...
        setFeedback("idle");
        try {
          await scope.set(field, desired);
          const accepted = scope.getSnapshot().value ?? {};
          if (!sameRoute(accepted[field], desired)) throw new Error("Host refused " + field);
          setFeedback("saved");
          setManual(false);
        } catch (_) {
          setFeedback("error");
    ...
    function SettingsForm({ t, configScope, loadCatalog }) {
      ...
      return e("section", { style: { display: "flex", flexDirection: "column", gap: 16 } },
        e(SummarizerPicker, {
          ...
          field: "summarizationRoute", optional: false,
          headingKey: "summarizerHeading", hintKey: "summarizerHint",
        }),
        e(SummarizerPicker, {
          t, scope: configScope, snapshot, catalogState, reloadCatalog,
          field: "fallbackSummarizationRoute", optional: true,
