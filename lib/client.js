...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const SUMMARIZER_FIELDS = [
      { key: "provider", labelKey: "summarizationProvider", placeholder: "openai" },
      { key: "model", labelKey: "summarizationModel", placeholder: "gpt-5.6-sol" },
    ];
    ...
    const zh = {
      ...
      summarizerHint: "Provider 和 Model 都留空时跟随当前主 Agent；指定独立压缩模型时必须两项同时填写。WebUI 会把两项作为一个原子路由一次保存。",
    ...
    const en = {
      ...
      summarizerHint: "Leave both Provider and Model empty to follow the current Agent. A dedicated route requires both; WebUI writes the pair atomically as one route setting.",
    ...
    function validateDraft(draft) {
      ...
      const route = normalizeRoute(resolved.summarizationRoute);
      if ((route.provider.length === 0) !== (route.model.length === 0)) return "invalid";
    ...
    function SettingsForm({ t, configScope }) {
      ...
      const updateRoute = (field, value) => {
        setDraft((current) => {
          const base = resolvedDraft(current);
          return {
            ...base,
            summarizationRoute: { ...base.summarizationRoute, [field]: value },
          };
        });
        setDirty(true);
        setFeedback("idle");
      ...
      const save = async () => {
        ...
        const desiredRoute = normalizeRoute(resolved.summarizationRoute);
        setBusy(true);
        setFeedback("idle");
        try {
          let accepted = scope.getSnapshot().value ?? {};
          if (!sameRoute(accepted.summarizationRoute, desiredRoute)) {
            await scope.set("summarizationRoute", desiredRoute);
            accepted = scope.getSnapshot().value ?? {};
            if (!sameRoute(accepted.summarizationRoute, desiredRoute)) throw new Error("Host refused summarizationRoute");
          }
      ...
      const summarizerRows = SUMMARIZER_FIELDS.map((field) => e("div", { key: field.key, style: rowStyle },
        e("label", { style: labelStyle }, t(field.labelKey)),
        e("span", { style: hintStyle }, t(field.labelKey + "Hint")),
        e("input", {
          type: "text",
          spellCheck: false,
          autoCapitalize: "none",
          ...
          style: inputStyle,
          value: String(resolved.summarizationRoute[field.key] ?? ""),
          disabled: !snapshot.writable || busy,
          onChange: (event) => updateRoute(field.key, event.target.value),
        })));

      const policyRows = POLICY_FIELDS.map((field) => {
