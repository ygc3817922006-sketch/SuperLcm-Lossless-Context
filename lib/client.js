 ...
 * DSH UI primitives.
 *
 * 2026-09-02：主/备用摘要路由改为由宿主模型目录驱动的原子下拉框。
 * 2026-09-02: the primary and backup summarizer routes use atomic dropdowns
 * fed by the Host model catalog (`remote.session.modelCatalog`, the same
 * directory the main model picker uses) and save on change; rolling knobs
 * remain under a collapsed "advanced" section with plainer labels.
 */
"use strict";
...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // dsh 0.1.7: configForms.get() 按宿主 profile 的引擎条目 id 寻址。
    // 条目 id 由部署方自定（示例用 SuperLcm-engine，本机用 SuperLcm-compaction），
    // 所以运行时从 describe 镜像里发现挂载了本插件的条目，而不是硬编码。
    const ENGINE_PLUGIN_NAME = "SuperLcm";
    const LOCALE_NAMESPACE = "settings.SuperLcm";
    const FOLLOW_AGENT = "";
    const ROUTE_SEPARATOR = "\t";

    // dsh 0.1.7: Config 字段名即表单字段名。旧 settings 区的
    // summarizationRoute{provider,model} 拆为宿主 Config 的两个标量字段。
    const ROUTE_FIELDS = Object.freeze({
      primary: Object.freeze(["summarizationProvider", "summarizationModel"]),
      fallback: Object.freeze(["fallbackSummarizationProvider", "fallbackSummarizationModel"]),
    });
    ...
    const zh = {
      ...
      summarizerHint: "后台压缩主模型。必须明确选择，不会回退到主 Agent；主模型失败时仅尝试下方备用模型。",
      fallbackSummarizerHeading: "备用压缩模型（可选）",
      fallbackSummarizerHint: "主压缩模型失败后只重试一次；未配置时保留原错误，不会回退到主 Agent。",
      fallbackDisabled: "不使用备用模型",
      ...
      manualModel: "模型 ID",
      manualHint: "提供方与模型必须同时填写；后台压缩不会跟随主 Agent。",
      manualApply: "应用",
      ...
      foldBatchTokensHint: "旧历史达到该规模即可后台准备摘要；接近上下文压力线时才换入。默认 64k。",
      ...
      invalid: "参数无效：单次压缩最少释放 ≤ 常规压缩批次；强制压缩阈值 > 开始压缩阈值。",
      invalidRoute: "压缩模型必须同时填写提供方和模型。",
      duplicateRoute: "备用模型必须与主压缩模型不同。",
      routeRequired: "请选择独立后台压缩模型",
      loading: "配置加载中…",
      unavailable: "设置服务不可用，暂时无法编辑。",
      readonly: "当前为只读模式。",
      modeNote: "所有自动压缩固定使用 rolling 后台模式；本页字段可热更新。",
    };
    ...
    const en = {
      ...
      summarizerHint: "Primary model for background compaction. It must be explicit, never falls back to the main Agent, and uses the backup below only after failure.",
      fallbackSummarizerHeading: "Backup summarizer model (optional)",
      fallbackSummarizerHint: "Retry once only after the primary fails. When unset, preserve the original error; never fall back to the main Agent.",
      fallbackDisabled: "No backup model",
      ...
      manualHint: "Provider and model are both required; background compaction never follows the main Agent.",
      manualApply: "Apply",
      ...
      foldBatchTokensHint: "Prepare this much old history in the background; replace it only near the context pressure line. Default 64k.",
      ...
      invalid: "Invalid: minimum freed ≤ routine batch; force threshold > start threshold.",
      invalidRoute: "A summarizer route requires both provider and model.",
      duplicateRoute: "The backup must differ from the primary summarizer.",
      routeRequired: "Choose a dedicated background model",
      loading: "Loading configuration…",
      unavailable: "Settings service unavailable; editing is disabled.",
      readonly: "Read-only mode.",
      modeNote: "All automatic compaction uses non-blocking rolling background mode; these fields apply live."
    };
    ...
    const labelStyle = { fontSize: 14, lineHeight: "20px", fontWeight: 600, color: "var(--dsw-alias-label-primary)" };
    ...
    // 控件一律只写长属性。`font:` 简写排在 fontSize 之后会把字号重置成继承值
    // （主题的 --dsh-content-font-size），控件就会比官方大一截。
    const inputStyle = {
    ...
    const linkButtonStyle = {
      ...
      color: "var(--dsw-alias-label-tertiary)", cursor: "pointer", textDecoration: "underline",
    };
    ...
    // dsh 0.1.7: 路由在宿主 Config 里是两个标量字段，这里按组读取。
    function routeOf(section, which) {
      const value = section ?? {};
      const [providerField, modelField] = ROUTE_FIELDS[which];
    ...
    function sameField(field, left, right) {
      if (field.kind === "int") return Number(left) === Number(right);
      return left === right;
    }
    ...
    function validatePolicy(draft) {
      ...
      const integerKeys = ["tailCount", "minRetainTokens", "pressureFoldTokens", "foldBatchTokens", "softActiveTokens", "hardActiveTokens"];
      for (const key of integerKeys) {
        const value = Number(resolved[key]);
        if (!Number.isInteger(value)) return "invalid";
        if (key === "minRetainTokens") {
      ...
      if (resolved.foldTiming !== "background") return "invalid";
      return null;
    }

    /** 设置作用域上的快照存储钩子；不可用时返回占位值 / Snapshot-store hook over a settings scope (or an unavailable stand-in). */
    // dsh 0.1.7: configForms.describe() 返回的是镜像（getSnapshot/subscribe/ensure），
    // 不是命名空间数组；条目 id 由部署方自定，所以从镜像里按 schema 特征发现。
    const ENGINE_FIELD_MARKER = "summarizationProvider";

    function findEngineEntry(describeFace) {
      let snapshot;
      try {
        snapshot = describeFace.getSnapshot();
      } catch (_) {
        return undefined;
      }
      const namespaces = (snapshot && snapshot.view && snapshot.view.namespaces) || [];
      for (const ns of namespaces) {
        let serialized = "";
        try {
          serialized = JSON.stringify(ns.schema);
        } catch (_) {
          serialized = "";
        }
        if (serialized.indexOf(ENGINE_FIELD_MARKER) >= 0) return ns.ns;
      }
      return undefined;
    }

    // 镜像首次是 idle，需要 ensure() 才会发起读取；解析到条目后订阅其变化。
    function useEngineScope(describeFace, configForms) {
      const [scope, setScope] = React.useState(null);
      React.useEffect(() => {
        if (!describeFace || !configForms) return undefined;
        let current;
        const sync = () => {
          const id = findEngineEntry(describeFace);
          if (id === current) return;
          current = id;
          setScope(id === undefined ? null : configForms.get(id));
        };
        sync();
        if (typeof describeFace.ensure === "function") {
          Promise.resolve(describeFace.ensure()).then(sync).catch(() => undefined);
        }
        const off = describeFace.subscribe(sync);
        return typeof off === "function" ? off : undefined;
      }, [describeFace, configForms]);
      return scope;
    }
    ...
    function SummarizerPicker({ t, scope, snapshot, catalogState, reloadCatalog, field, optional, headingKey, hintKey }) {
      ...
      const current = routeOf(snapshot.value, field);
      ...
      const commit = async (route) => {
        ...
        if (!routeValid(route) && !(optional && routeKey(route) === FOLLOW_AGENT)) { setFeedback("invalidRoute"); return; }
        const desired = normalizeRoute(route);
        const otherField = field === "primary" ? "fallback" : "primary";
        const other = routeOf(snapshot.value, otherField);
        if (routeValid(desired) && routeValid(other) && sameRoute(desired, other)) { setFeedback("duplicateRoute"); return; }
        if (sameRoute(desired, current)) { setFeedback("idle"); return; }
        setBusy(true);
        setFeedback("idle");
        try {
          const [providerField, modelField] = ROUTE_FIELDS[field];
          await scope.mutate([
            { op: "set", path: [providerField], value: desired.provider },
            { op: "set", path: [modelField], value: desired.model },
          ]);
          const accepted = scope.getSnapshot().value ?? {};
          if (!sameRoute(routeOf(accepted, field), desired)) throw new Error("Host refused " + field);
          setFeedback("saved");
          setManual(false);
        } catch (_) {
          setFeedback("error");
      ...
      const followLabel = optional ? t("fallbackDisabled") : t("routeRequired");
      const options = currentKey === FOLLOW_AGENT || optional
        ? [e("option", { key: "__empty", value: FOLLOW_AGENT, disabled: !optional }, followLabel)]
        : [];
      if (currentIsCustom) {
      ...
      return e("div", { style: sectionStyle },
        e("div", { style: labelStyle }, t(headingKey)),
        ...
        e("div", { style: controlsRowStyle },
          ...
          feedback === "duplicateRoute" ? e("span", { style: errorStyle }, t("duplicateRoute")) : null),
        manualForm);
    }
    ...
    function AdvancedForm({ t, scope, snapshot }) {
      ...
      const save = async () => {
        ...
        try {
          ...
          const acceptedBefore = before.value ?? {};
          const desiredFields = POLICY_FIELDS.map((field) => {
            const raw = resolved[field.key];
            return { field, value: field.kind === "select" ? raw : Number(raw) };
          });
          const ops = desiredFields
            .filter(({ field, value }) => !sameField(field, acceptedBefore[field.key], value))
            .map(({ field, value }) => ({ op: "set", path: [field.key], value }));
          if (ops.length > 0) await scope.mutate(ops, before.revision);
          const accepted = scope.getSnapshot().value ?? {};
          for (const { field, value } of desiredFields) {
            if (!sameField(field, accepted[field.key], value)) throw new Error("Host refused " + field.key);
          }
          setDraft(accepted);
          setDirty(false);
          setFeedback("saved");
    ...
    function SettingsForm({ t, configScope, loadCatalog, pending }) {
      ...
      // scope 未解析（镜像还在读）时显示加载中，不要误报「设置服务不可用」。
      if (pending) return e("div", { style: noteStyle }, t("loading"));
      ...
      return e("section", { style: { display: "flex", flexDirection: "column", gap: 16 } },
        e(SummarizerPicker, {
          t, scope: configScope, snapshot, catalogState, reloadCatalog,
          field: "primary", optional: false,
          headingKey: "summarizerHeading", hintKey: "summarizerHint",
        }),
        e(SummarizerPicker, {
          t, scope: configScope, snapshot, catalogState, reloadCatalog,
          field: "fallback", optional: true,
          headingKey: "fallbackSummarizerHeading", hintKey: "fallbackSummarizerHint",
        }),
    ...
    function SuperLcmPluginConfig({ t, describeFace, configForms, loadCatalog, view }) {
      if (t === undefined) throw new Error("SuperLcm configuration requires its translation function");
      // 必须在任何提前 return 之前调用 hook，保持 hook 顺序稳定。
      const configScope = useEngineScope(describeFace, configForms);
      ...
      return e("section", { style: cardBodyStyle },
        ...
        e(SettingsForm, { t, configScope, loadCatalog, pending: configScope === null }));
    }
    ...
    function apply(ctx) {
      ...
      // dsh 0.1.7: 旧 settings 作用域服务已改名为 configForms，bind({namespace}) 改为
      // get(entryId)。条目 id 从 describe 镜像发现：schema 含本插件 volatile 字段的
      // 条目即引擎条目（summarizationProvider 是 SuperLcm 独有键）。
      const describeFace = ctx.configForms.describe();
      // 提前发起一次读取，页面打开时通常已经拿到命名空间列表。
      if (typeof describeFace.ensure === "function") {
        Promise.resolve(describeFace.ensure()).catch(() => undefined);
      }
      const loadCatalog = () => ctx.remote.session.modelCatalog();
      ctx.slots.inject("plugins.bundle.config", () => ctx.slots.register({
        name: "plugins.bundle.config",
        key: "SuperLcm",
        inject: () => ({ t, describeFace, configForms: ctx.configForms, loadCatalog }),
