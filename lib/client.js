...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function apply(ctx) {
      ...
      const discoverEngineEntry = () => {
        const described = ctx.configForms.describe();
        const namespaces = described?.namespaces ?? described ?? [];
        for (const view of namespaces) {
          const schema = view.schema;
          const dict = schema && typeof schema === "object" ? schema.dict : undefined;
          if (dict && dict.summarizationProvider !== undefined) return view.ns;
        }
        return undefined;
      };
      const configScopeFor = (entryId) => (
        entryId === undefined ? null : ctx.configForms.get(entryId)
      );
      const loadCatalog = () => ctx.remote.session.modelCatalog();
      ctx.slots.inject("plugins.bundle.config", () => ctx.slots.register({
        name: "plugins.bundle.config",
        key: "SuperLcm",
        inject: () => ({ t, configScope: configScopeFor(discoverEngineEntry()), loadCatalog }),
