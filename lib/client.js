 ...
 * model picker uses) and saves on change; the rolling knobs moved under a
...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const zh = {
      ...
      summarizerHint: "后台压缩专用模型。必须明确选择，不会回退到主 Agent；选择后立即生效。",
    ...
    const en = {
      ...
      summarizerHint: "Dedicated model for background compaction. It must be explicit and never falls back to the main Agent. Applies immediately.",
      ...
      invalidRoute: "Background compaction requires both provider and model.",
    ...
    function SummarizerPicker({ t, scope, snapshot, loadCatalog }) {
      ...
      const followLabel = t("routeRequired");
      const options = currentKey === FOLLOW_AGENT
        ? [e("option", { key: "__required", value: FOLLOW_AGENT, disabled: true }, followLabel)]
        : [];
      if (currentIsCustom) {
